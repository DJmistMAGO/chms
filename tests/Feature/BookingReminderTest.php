...
it('sends only the 3 hour warning for bookings already inside the 3 hour window', function () {
    Mail::fake();
    ...
    Mail::assertQueued(BookingExpirationWarning::class, function ($mail) use ($booking) {
        return $mail->booking->is($booking) && $mail->remainingTime === '3 hours';
    });

    Mail::assertQueuedCount(1);

    expect(BookingReminder::where('booking_id', $booking->id)->where('type', '3h')->exists())->toBeTrue();
    expect(BookingReminder::where('booking_id', $booking->id)->where('type', '12h')->exists())->toBeFalse();
});
