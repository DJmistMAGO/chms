...
@section('content')
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        ...
        <div class="mb-5 flex items-center justify-between lg:mb-7">
            ...
            <button type="button" onclick="openAddStaffModal()" class="inline-flex items-center gap-2 rounded-lg bg-amber-600 px-4 py-2 text-sm font-medium text-amber-50 transition hover:bg-amber-700 active:bg-amber-800" >
        ...
        <div class="mb-5 border-b border-gray-200 dark:border-gray-800">
            <nav class="-mb-px flex gap-6" aria-label="User types">
                <button type="button" id="staffTab" onclick="switchUserTab('staff')" class="user-tab border-b-2 border-amber-600 px-1 pb-3 text-sm font-medium text-amber-600 transition" >
                    Staff
                    <span class="ml-1.5 rounded-full bg-amber-100 px-2 py-0.5 text-xs text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">
                        {{ $staffUsers->count() }}
                    </span>
                </button>


                <button type="button" id="clientTab" onclick="switchUserTab('client')" class="user-tab border-b-2 border-transparent px-1 pb-3 text-sm font-medium text-gray-500 transition hover:border-gray-300 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" >
                    Clients
                    <span class="ml-1.5 rounded-full bg-green-100 px-2 py-0.5 text-xs text-green-700 dark:bg-green-900/40 dark:text-green-300">
                        {{ $clientUsers->count() }}
                    </span>
                </button>
            </nav>
        ...
        <div class="mb-5 relative">
            ...
            <input type="text" id="search-input" placeholder="Search by name, email, phone number, or address" oninput="filterTable()" class="w-full rounded-xl border border-gray-200 bg-gray-50 py-2.5 pl-10 pr-4 text-sm text-gray-700 transition focus:border-yellow-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-yellow-400/20 dark:border-gray-700 dark:bg-gray-800/50 dark:text-white" >
        </div>


        <div id="addStaffModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 backdrop-blur-sm" >
                ...
                <form action="{{ route('user-management.addStaff') }}" method="POST" autocomplete="off" >
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" required autocomplete="off" class="w-full rounded-lg border border-gray-300 px-4 py-2 dark:border-gray-700 dark:bg-gray-800 dark:text-white" >
                        </div>
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" onclick="closeAddStaffModal()" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-100 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800" >
                            Cancel
                        </button>

                        <button type="submit" class="rounded-lg bg-amber-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-amber-700" >
                            Create User
                        </button>
                    </div>
                </form>
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse ($staffUsers as $user)
                            <tr class="user-row transition hover:bg-gray-50 dark:hover:bg-white/[0.02]" data-search="{{ strtolower($user['name']) }} {{ strtolower($user['email']) }} {{ strtolower($user['phone'] ?? '') }} {{ strtolower($user['address'] ?? '') }}" >
                                <td class="py-3 pr-4">
                                ...
                                <td class="py-3">
                                    ...
                                    @else
                                        <span class="inline-flex items-center rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-700 dark:bg-red-900/40 dark:text-red-300">
                                            Inactive
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="py-3 text-sm">
                                    <div class="inline-flex flex-wrap items-center gap-2">
                                        ...
                                        @if ($user['status'] === 'active')
                                            <button type="button" onclick="openStatusModal('{{ $user['id'] }}', 'deactivate')" class="inline-flex items-center gap-1 rounded-lg bg-red-600 px-3 py-1 text-xs font-medium text-white transition hover:bg-red-700 active:bg-red-800" >
                                                Deactivate
                                            </button>
                                        @else
                                            <button type="button" onclick="openStatusModal('{{ $user['id'] }}', 'activate')" class="inline-flex items-center gap-1 rounded-lg bg-green-600 px-3 py-1 text-xs font-medium text-white transition hover:bg-green-700 active:bg-green-800" >
                                                Activate
                                            </button>
                                        @endif
                                        <form action="{{ route('user-management.reset-password', $user['id']) }}" method="POST" class="inline-flex" >
                                            @csrf
                                            <button type="submit" class="inline-flex items-center gap-1 rounded-lg bg-yellow-600 px-3 py-1 text-xs font-medium text-white transition hover:bg-yellow-700 active:bg-yellow-800" >
                                                Reset Password
                                            </button>
                                        </form>
                                    </div>
                                ...
                                <td class="py-3 pr-4">
                                    <div class="flex items-center gap-3">
                                        @if($user['avatar'])
                                            <img src="{{ asset('storage/' . $user['avatar']) }}" alt="{{ $user['name'] }}'s Avatar" class="h-9 w-9 rounded-full object-cover" >
                                        @else
                                            <div class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-green-100 text-sm font-medium text-green-700 dark:bg-green-900/40 dark:text-green-300">
                                                {{ strtoupper(substr($user['name'], 0, 2)) }}
                                            </div>
                                        @endif
        ...
        <div id="statusModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm" >
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="closeStatusModal()" class="flex-1 rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-white/[0.05]" >
                        Cancel
                    </button>
                    <form id="statusForm" method="POST" class="flex-1" >
                        @csrf
                        <button type="submit" id="modalConfirmBtn" class="w-full rounded-lg px-4 py-2 text-sm font-medium text-white transition" >
                            Confirm
                        </button>
                    </form>
                </div>
        ...
        <div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="editModalTitle" >
            <div class="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-gray-900">
                <div class="relative bg-gradient-to-br from-amber-50 to-white px-6 pb-5 pt-6 dark:from-gray-800 dark:to-gray-900">
                    <button type="button" onclick="closeEditModal()" aria-label="Close" class="absolute right-5 top-5 flex h-8 w-8 items-center justify-center rounded-full text-gray-400 transition hover:bg-black/5 hover:text-gray-600 dark:hover:bg-white/10 dark:hover:text-gray-300" >
                    ...
                    <div class="flex items-center gap-4">
                        <div id="editAvatar" class="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-amber-100 font-serif text-xl font-semibold text-amber-700 dark:bg-amber-900/40 dark:text-amber-300" > </div>
                        <div>
                ...
                <div class="px-6 py-6">
                    <form id="editForm" method="POST" class="grid grid-cols-1 gap-5 sm:grid-cols-2" >
                        ...
                        <div>
                            ...
                            <input id="editName" name="name" type="text" required class="mt-1 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" >
                        </div>

                        <div>
                            <label for="editEmail" class="text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500" > Email </label>
                            <input id="editEmail" name="email" type="email" required class="mt-1 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" >
                        </div>
                        ...
                        <div class="mt-2 flex justify-end gap-3 sm:col-span-2">
                            <button type="button" onclick="closeEditModal()" class="rounded-xl border border-gray-200 px-5 py-2.5 text-sm font-medium text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.05]" >
                                Cancel
                            </button>
