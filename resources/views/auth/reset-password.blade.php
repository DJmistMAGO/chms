...
<head>
    ...
    <title>Set New Password - Caree Hotel</title>
...
<body class="min-h-screen flex items-center justify-center bg-cover bg-center relative"
    ...
    <div class="relative z-10 w-full max-w-sm mx-4">
        <div class="bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl px-8 py-9 card-enter">
            ...
            <h2 class="font-display text-3xl font-semibold text-center text-stone-800 tracking-wide mb-1">New Password</h2>
            <p class="text-center text-stone-400 text-xs mb-7 font-light leading-relaxed">
                Please enter your email and set your new account password below.
            </p>

            {{-- General Failure Alert (Shows WHY the reset failed) --}}
            @if ($errors->any())
            ...
            <form action="{{ route('password.update') }}" method="POST" class="space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <input
                        type="email"
                        name="email"
                        value="{{ request()->email ?? old('email', $email ?? '') }}"
                ...
                {{-- New Password --}}
                <div class="space-y-1">
                    <label class="block text-xs font-medium text-stone-500 tracking-widest uppercase">New Password</label>
                    <div class="relative">
                        <input
                            type="password"
                            name="password"
                ...
                <div class="space-y-1">
                    <label class="block text-xs font-medium text-stone-500 tracking-widest uppercase">Confirm Password</label>
                    <div class="relative">
                        <input
                            type="password"
                ...
                <button type="submit"
                    class="btn-primary w-full text-white py-3 mt-2 rounded-xl font-medium tracking-wide text-sm shadow-md">
                    Update Password
                </button>
            </form>
            ...
            <p class="text-center text-stone-400 text-xs mt-6">
                Return to
                <a href="{{ route('login') }}" class="text-amber-600 font-medium hover:text-amber-700 transition">Sign In</a>
            </p>
